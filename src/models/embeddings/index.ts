...
export async function getCoordinates(inputs: string[], apiKey: string): Promise<number[][] | undefined> {
  if (inputs.length < 1) {
  ...
  const embeddingsResponse = embedModel.getEmbeddings(inputs)
    .catch(
      (err) => {
        console.error("Error while retrieving node embeddings from OpenAI: " + err)
      }
